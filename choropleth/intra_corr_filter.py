...
intracorr_cutoff = float(sys.argv[1])
...
for window in dates:
    ...
    try:
        ...
        iota = iota.drop(columns=['symptom', 'pair']).reset_index().groupby(by='state').apply(lambda x: x.loc[x['corr'] > intracorr_cutoff]).drop(columns=['state']).reset_index().drop(columns=['level_1'])
        iota['window'] = window
