...
def plot(counter, out, log, rotate):
	tuples = sorted(counter.items(), key=lambda x: -x[0])
	new_tuples = []
	gt25 = 0
	for t in tuples:
		if t[0] <= 25:
			...
			gt25 += t[1]
	new_tuples.insert(0, ('gt25', gt25))
	labels = [t[0] for t in new_tuples]
	counts = [t[1] for t in new_tuples]
...
def parse_counts(counts, cutoff):
	...
	for row in counts:
		...
		if corr > cutoff:
			all_members.append( len(members))
